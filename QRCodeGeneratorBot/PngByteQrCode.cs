...
public sealed class PngByteQrCode : AbstractQrCode, IDisposable
{
    ...
    private byte[] DrawScanlines(int pixelsPerModule, bool drawQuietZones)
    {
        ...
        var bytesPerScanline =
            (matrixSize * pixelsPerModule + 7) / 8 +
            1; 
        ...
        for (var y = 0; y < matrixSize; y++)
        {
            ...
            {
                Array.Copy(scanlines, scanlineOffset, scanlines, scanlineOffset + copyCount * bytesPerScanline,
                    bytesPerScanline);
            }
        }
    ...
    private sealed class PngBuilder : IDisposable
    {
        ...
        private readonly static uint[] CrcTable =
        {
            ...
            0x94643B84, 0x0D6D6A3E, 0x7A6A5AA8, 0xE40ECF0B, 0x9309FF9D, 0x0A00AE27, 0x7D079EB1, 0xF00F9344, 0x8708A3D2,
            ...
            0xE2B87A14, 0x7BB12BAE, 0x0CB61B38, 0x92D28E9B, 0xE5D5BE0D, 0x7CDCEFB7, 0x0BDBDF21, 0x86D3D2D4, 0xF1D4E242,
            0x68DDB3F8, 0x1FDA836E, 0x81BE16CD, 0xF6B9265B, 0x6FB077E1, 0x18B74777, 0x88085AE6, 0xFF0F6A70, 0x66063BCA,
            0x11010B5C, 0x8F659EFF, 0xF862AE69, 0x616BFFD3, 0x166CCF45, 0xA00AE278, 0xD70DD2EE, 0x4E048354, 0x3903B3C2,
            0xA7672661, 0xD06016F7, 0x4969474D, 0x3E6E77DB, 0xAED16A4A, 0xD9D65ADC, 0x40DF0B66, 0x37D83BF0, 0xA9BCAE53,
            0xDEBB9EC5, 0x47B2CF7F, 0x30B5FFE9, 0xBDBDF21C, 0xCABAC28A, 0x53B39330, 0x24B4A3A6, 0xBAD03605, 0xCDD70693,
            0x54DE5729, 0x23D967BF, 0xB3667A2E, 0xC4614AB8, 0x5D681B02, 0x2A6F2B94, 0xB40BBE37, 0xC30C8EA1, 0x5A05DF1B,
            0x2D02EF8D
        }; 
        ...
        public byte[]? GetBytes()
        {
            ...
            while (bytes is not null && chunkOffset < bytes.Length)
            {
                var dataLength = (bytes[chunkOffset] << 24) | (bytes[chunkOffset + 1] << 16) |
                                 (bytes[chunkOffset + 2] << 8) | bytes[chunkOffset + 3];
...
{
    public static byte[]? GetQrCode(string plainText, int pixelsPerModule, byte[] darkColorRgba, byte[] lightColorRgba,
